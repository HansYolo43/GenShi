...
public class GenerateCardDataAccessInterFace {
    ...
    public String generateDescription(String characterName, String attackName, String defenseName) throws IOException {
        String prompt = String.format("Describe a character named %s with %s defense stat %d and attack names %s with "
                        +
                        "stats %d or %d in the %s theme:", characterName, defenseName, attackName
                , themePrompt);
        HttpURLConnection con = createConnection();
    ...
    public void generateAndSaveCharacters(int numCharacters, String filePath) throws IOException {
        ...
        for (int i = 0; i < numCharacters; i++) {
            ...
            dataAccessObject.addCardbyinfo(characterName, description, "Not Assigned",level, affinity, basehp , basedef , baseatk , basecrit );
